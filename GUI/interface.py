...
class JanelaPrincipal:
    ...
    def tela_de_aviso(self, frame_principal, msg):
    ...
    def __conecxao_db(self):
        conn = None
        try:
            conn = sqlite3.connect("database/pessoas.db", detect_types = sqlite3.PARSE_DECLTYPES)
    ...
    def botao_registrar_clicado(self, frame_principal):
        ...
        self.texto_email.place(relx=0.325, rely=0.117, height=20, relwidth=0.461)
        ...
        self.label_email.place(relx=0.195, rely=0.117, height=21, width=65)
        ...
        self.texto_cpf.place(relx=0.325, rely=0.188,height=21, relwidth=0.461)
        ...
        self.label_cpf.place(relx=0.195, rely=0.188, height=21, width=65)
        ...
        self.opc_generico = ttk.Radiobutton(self.frame_principal, variable = self.cargo_selecionado, value = 1)
        self.opc_generico.place(relx=0.292, rely=0.282, relwidth=0.112, relheight=0.0, height=21)
        self.opc_generico.configure(text='''Genérico''')
        self.opc_generico.configure(cursor="hand2")

        self.opc_diretor = ttk.Radiobutton(self.frame_principal, variable = self.cargo_selecionado, value = 2)
        self.opc_diretor.place(relx=0.455, rely=0.282, relwidth=0.112, relheight=0.0, height=21)
        self.opc_diretor.configure(text='''Diretor''')
        self.opc_diretor.configure(cursor="hand2")

        self.opc_ministro = ttk.Radiobutton(self.frame_principal, variable = self.cargo_selecionado, value = 3)
        self.opc_ministro.place(relx=0.601, rely=0.282, relwidth=0.112, relheight=0.0, height=21)
        ...
        self.webcam_frame.place(relx=0.227, rely=0.390, relheight=0.481, relwidth=0.56)
        ...
        try:
            ...
            self.tela_de_aviso(frame_principal, "Ocorreu um erro! Sua webcam não está disponível.")
    ...
    def botao_confirmar_registro_clicado(self, frame_principal):
        # Tem que salvar em variável, porque toda vez que tu da 'get()' ele limpa o local de origem.
        nome, email, cpf = self.texto_nome.get(), self.texto_email.get(), self.texto_cpf.get()

        if self.validar_dados(nome, email, cpf):
            for child in frame_principal.winfo_children():
                child.destroy()

            self.salvar_database(nome, email, cpf)

            self.tela_de_aviso(frame_principal, "Você foi registrado com sucesso!")
        else:
            self.tela_de_aviso(frame_principal, "Você inseriu dados incorretos.")
    ...
    def salvar_database(self, nome, email, cpf):
        def adapt_array(arr):
            out = io.BytesIO()
            np.save(out, arr)
            out.seek(0)
        ...
        cur = conn.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS registros (nome text, email text, cpf int, arr array)")
        cur.execute("INSERT INTO registros (nome, email, cpf, arr) VALUES (?, ?, ?, ?)", (nome, email, cpf, self.frame, ))
        conn.commit()
        conn.close()
